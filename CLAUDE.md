...
- `--flat` — Resolve modifiers into individual category codes. Each modifier combination produces a new terminal category that inherits parent metadata. For categories with multiple modifiers, only fully-resolved codes are emitted (no partial application). Parent categories gain a `mod_codes` field listing their resolved codes. Resolved categories get a `label_long` joining parent label + modifier value labels with `: `. The top-level modifier definitions remain in the output.


### Piping to jq

When writing to stdout (no `-o`), pipe to `jq` to extract subsets:

```bash
cclaml icd10gm2025.xml | jq '.chapters'                                           # only chapters
cclaml icd10gm2025.xml | jq '.blocks'                                             # only blocks
